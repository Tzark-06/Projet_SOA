...
public class LightController {
	...
	public ArrayList<LightDataAcquisition> lightListRoom(@PathVariable String room){
		...
		for(int i = 0; i < LightRoomAcquisitionList.getLightDataList().size(); i++){
			...
			if(LightRoomAcquisitionList.getLightDataList().get(i).getRoomName() == room){
	...
	public void addStudentToList(@PathVariable String room, @PathVariable int value, @PathVariable String unit){
